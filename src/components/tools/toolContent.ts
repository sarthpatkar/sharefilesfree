...
export const TOOL_CONTENT: Record<string, ToolContent> = {
  ...
  "edit-pdf": {
    intro: [
      "Two different searches lead here and they want the same thing. \"Edit PDF\" is usually someone who needs to put something ON a page — a signature, a date, a corrected figure, a note for whoever reads it next. \"Annotate PDF\" is the same person using the word a lawyer or a lecturer would. So this is one tool rather than two: text, highlighting, underline, strikethrough, boxes, ovals, lines, arrows, whiteout, a freehand pen, images, clickable links, form fields, and a way to type over the document's own text.",
      "It runs entirely in your browser, which matters more here than for almost anything else on the site. The documents people sign, initial or scribble a correction on are contracts, tenancy agreements, medical forms and passport scans — the exact category you would least like to hand to a stranger's server for processing. Nothing is uploaded, so there is nothing to hand over, and nothing to cap: no file size limit, no page limit, and no ration on how many documents you get through in an hour.",
      "The mark-up reads the page's own text rather than following your hand. Drag across a sentence and the highlight snaps to the lines it covers, so it comes out ruled and even instead of wobbling wherever the mouse went. Underline, strikethrough and links work the same way. On a scanned page there is no text to snap to, and it says so rather than pretending.",
      "If a document has a real form in it, that is filled through its actual fields rather than by painting text on top — so the answers stay editable, stay machine-readable for whoever receives them, and land exactly where the field is.",
    ],
    steps: [
      "Open the PDF. The first page is drawn on your device, sized to fit the column, with the rest a click away. If the file contains a form, its fields are listed underneath the page straight away.",
      "Pick a tool. Text and Signature place a box where you click. Highlight, underline, strikethrough and Link are drag-across-the-words. Pen, line, arrow, box, oval and whiteout are drag-to-draw. Image asks for a picture first, then a box to fit it into.",
      "To correct something the document already says, use Retype: click the line and it is covered with the colour of the paper around it and opened for typing, with the size and the family of the original already matched.",
      "Set the colour, size or line weight before you draw — the controls above the page change with the tool you have chosen. Stamps put today's date, APPROVED, DRAFT or CONFIDENTIAL in one click.",
      ...
      "Use Select to fix things: drag a mark to move it, drag a corner handle to resize it, double-click text to retype it, and Duplicate to stamp a signature onto another page. ⌘Z, ⇧⌘Z, ⌘D and Delete all work.",
      "Save the edited PDF, or hand it straight to the transfer page to send it to someone.",
    ],
    limits: [
      "Retype covers and replaces; it does not re-flow. A line is patched over and set again in one of the built-in fonts at the original size, which is right for a date, a name, a price or a typo. It is not a word processor: make the replacement much longer than the original and it will run past where the line used to end, because nothing here knows where the paragraph was supposed to wrap. Editors that do re-flow exist — Sejda is the one worth knowing about — and they warn about the same thing in multi-column layouts.",
      "Retype matches the family of the original — serif, sans or monospace — but not the exact typeface, because the document's own font is embedded for the glyphs it already contains rather than for new ones. Weight and slope start plain; the B and I buttons are one click away. On a body of plain text the join is usually invisible; on a distinctive branded font it will not be.",
      ...
      "Whiteout, and any filled box over sensitive text, hides it from view but does NOT remove it. The words are still in the file and can be copied straight out from underneath. This is not a redaction tool, and no tool that draws a box on top of text is one. The same is true of a retyped line: the original text is still in the file underneath the patch.",
      "Mark-up and Retype work only where the page has real text. A scan or a photographed document is a picture of words, not words, so highlighting falls back to the shape you dragged and Retype finds nothing at all — run OCR first. Text set at an angle, such as a diagonal watermark, is skipped for the same reason.",
      "Form fields cannot be added to a rotated page. The PDF format positions them in unrotated space, so one placed on a turned page would sit sideways; the tool says so rather than producing a broken form. Rotate the page upright first.",
      "Links accept http, https and mailto addresses only. A PDF link can carry any URI and readers will follow it, so the rest are refused rather than written into a file you are about to send to somebody.",
      "New text uses the fonts built into the PDF format, which cover Latin letters, digits and punctuation. Greek, Cyrillic, Chinese, Japanese, Korean and emoji have no place in those fonts — the tool tells you before you place the text rather than after. Embedding a Unicode font would mean a multi-megabyte download on every visit.",
      "A password-protected PDF has to be unlocked first, since the pages cannot be drawn while they are encrypted.",
      "The typed signature is a name set in italic, not handwriting. There is no script font, because shipping one would cost every visitor a download. For something that looks handwritten, sign with the pen or place a photograph of your signature.",
    ],
    faqs: [
      ...
      { q: "Is whiteout the same as redacting?", a: "No, and treating it as though it were is how confidential information leaks. The white rectangle sits above the text; the text itself is untouched and anyone can select and copy it, or read it out of the file directly. The same goes for a retyped line — the original is still under the patch. Real redaction removes the underlying content. If a document must be safe to release, delete the sensitive text at the source and produce a fresh PDF." },
      { q: "Can I move or resize something after placing it?", a: "Yes. Switch to Select, then drag a mark to move it or drag one of its corner handles to resize it — text rescales its font size, a drawing scales as a whole, and a line gets a handle at each end. Double-clicking a piece of text reopens it for retyping. Undo and redo are there for everything else." },
      ...
      { q: "What happens to pages that were scanned in sideways?", a: "They are handled. A page can carry its own rotation, and marks are positioned through the same machinery the viewer uses, so text lands upright and in the place you clicked rather than rotated into a corner. The one exception is form fields, which the PDF format will not let anyone place on a rotated page. If the whole document is the wrong way up, Rotate PDF will fix that first." },
      PRIVACY_FAQ,
      NO_LIMIT_FAQ,
      FREE_FAQ,
    ],
