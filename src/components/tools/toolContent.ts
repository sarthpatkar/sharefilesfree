...
export const TOOL_CONTENT: Record<string, ToolContent> = {
  ...
  "edit-pdf": {
    intro: [
      "Two different searches lead here and they want the same thing. \"Edit PDF\" is usually someone who needs to put something ON a page — a signature, a date, a corrected figure, a note for whoever reads it next. \"Annotate PDF\" is the same person using the word a lawyer or a lecturer would. So this is one tool rather than two: text, highlighting, underline, strikethrough, boxes, ovals, lines, arrows, whiteout, a freehand pen and images, placed anywhere on any page.",
      ...
      "The highlighter reads the page's own text rather than following your hand. Drag across a sentence and the highlight snaps to the lines it covers, so it comes out ruled and even instead of wobbling wherever the mouse went. Underline and strikethrough work the same way. On a scanned page there is no text to snap to, and it says so rather than pretending.",
    ],
    steps: [
      "Open the PDF. The first page is drawn on your device, sized to fit the column, with the rest a click away.",
      "Pick a tool. Text places a box where you click and types into it — several lines if you want. Highlight, underline and strikethrough are drag-across-the-words. Pen, line, arrow, box, oval and whiteout are drag-to-draw. Image asks for a picture first, then a box to fit it into.",
      ...
      "Use Select to fix things: drag a mark to move it, drag a corner handle to resize it, double-click text to retype it, and Duplicate to stamp a signature onto another page. Undo and redo cover the rest, and ⌘Z, ⇧⌘Z, ⌘D and Delete all work.",
      "Save the edited PDF, or hand it straight to the transfer page to send it to someone.",
    ],
    limits: [
      "It cannot retype the text that is already in the PDF. This adds new content on top of the page; it does not let you click into an existing paragraph and change a word. Some editors do manage that — Sejda is the one worth knowing about — and they do it honestly rather than by trickery, at the cost of caveats of their own: reflow gets unpredictable in multi-column layouts, it does not work on flattened PDFs where the text has become image paths, and font matching is approximate. It is a real feature we do not have yet.",
      ...
      "Whiteout, and any filled box over sensitive text, hides it from view but does NOT remove it. The words are still in the file and can be copied straight out from underneath. This is not a redaction tool, and no tool that draws a box on top of text is one.",
      "Highlighting snaps only where the page has real text. A scan or a photographed document is a picture of words, not words, so the mark falls back to the shape you dragged — run OCR first if you want it to snap. Text set at an angle, such as a diagonal watermark, is skipped for the same reason: a rectangle cannot honestly cover it.",
      ...
      "Marks are placed by hand. Apart from the text snapping there is no grid, no alignment guide and no form-field detection, so filling a long form is a manual job.",
    ],
    faqs: [
      { q: "Can I change the wording that is already in the document?", a: "Not here, no. This tool adds new content over the existing page, which is what most \"edit\" jobs actually need — a signature, a date, a correction, a note — but it will not let you retype an existing sentence. The reason it is hard is that a PDF stores glyphs at fixed coordinates and records nothing about where a sentence begins or ends, so lengthening a word means re-flowing a line the file never described. It is not impossible: Sejda does it, and does it well. If retyping existing paragraphs is the job, use that instead of fighting this." },
      { q: "How does this compare to Sejda or the other online editors?", a: "They can do more, and they can do it because they do the work on a server. That is also where their limits come from: Sejda's free tier caps you at a 50 MB file, a few dozen pages, and a small number of tasks per hour, and your document sits on their machines for a couple of hours before deletion. This tool runs in your browser, so there is no cap on size, no cap on pages, no queue, no task limit, and the file never leaves your device. The trade is real in both directions — pick the one whose trade-off suits the document in front of you. For a contract you would rather not upload anywhere, that is this one." },
      { q: "How do I sign a document?", a: "Two ways, and both are common. Choose the pen and sign with a finger, a stylus or a trackpad — zoom in first, it is much easier at 200%. Or photograph your signature on white paper, cut it out as a PNG with a transparent background, and place it with the image tool, which is the tidier option if you sign a lot of things. Either way, select the result and press Duplicate to stamp the same signature onto another page rather than drawing it again." },
