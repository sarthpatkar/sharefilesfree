...
export default function GlobalError({ reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <main className="mx-auto flex w-full max-w-md flex-1 flex-col items-center justify-center gap-4 px-4 py-20 text-center">
      ...
      <h1 className="font-display text-[1.5rem] tracking-[-0.015em] text-foreground">Something went wrong</h1>
      ...
      <button
        type="button"
        onClick={reset}
        className="inline-flex items-center justify-center bg-accent px-5 py-2.5 text-sm font-medium text-accent-foreground sff-block-ink sff-press hover:bg-accent-hover"
      >
        Try again
      </button>
    </main>
