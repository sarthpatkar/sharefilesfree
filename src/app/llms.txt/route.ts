...
Once a transfer connects, both devices show the same short verification code, derived from the two connection keys, so the two people can confirm nobody is between them without trusting the service. ShareFilesFree also warns a receiver when an arriving file is executable, and strips the bidirectional-override characters that let a file disguise its extension. It never stores a file: the transfer product moves a file directly between the sender's and receiver's browsers over a peer-to-peer (WebRTC) connection, and every tool listed above runs entirely in the visitor's own browser rather than uploading the file to be processed. Both devices need to be online at the same time to complete a peer-to-peer transfer — there is no inbox holding a file for later pickup. There is no account system, no sign-up, no watermark, and no file size limit on the transfer product. The site is free, funded by advertising rather than by charging users.
`;
